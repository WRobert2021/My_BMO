"""UI-toolkit-neutral recognizers for kiosk touch gestures."""
...
class HorizontalSwipeRecognizer:
    """Classify a press/release pair without depending on a UI toolkit."""
    ...
    def release(self, x: int, y: int) -> GestureKind:
        ...
        if abs(delta_x) >= self.minimum_distance and abs(delta_x) > abs(delta_y):
            if delta_x < 0:
...
__all__ = ["GestureKind", "HorizontalSwipeRecognizer"]
